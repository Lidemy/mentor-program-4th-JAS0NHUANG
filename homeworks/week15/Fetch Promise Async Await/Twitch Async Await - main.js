...
sendRequest(`${API_URL}/games/top?limit=6&offset=${topGameOffset}`, null)
  .then(text => {
    showTopGames(text)
  })

async function sendRequest(requestUrl, selectedGame) {
  let text 
  try {
...
function showTopGames(responseData) {
  ...
  if (firstLoad === true) {
    createDivs()
    sendRequest(`${API_URL}/streams/?limit=20&offset=${streamOffset - 20}`, '')
      .then(text => {
        showStreams(text)
      })
    firstLoad = false
